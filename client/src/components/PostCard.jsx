    ...
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl overflow-hidden hover:border-white/20 transition-all duration-300 hover:shadow-xl hover:shadow-black/20">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-white/70 text-sm font-medium">{post.username}</span>
          ...
          {currentUser?._id === post.userId && (
            <button onClick={handleDelete} className="text-white/20 hover:text-red-400 transition text-lg">🗑</button>
          )}
        </div>
        ...
        <div className="flex items-center gap-5 pt-2 border-t border-white/5">
          <button
            onClick={handleLike}
            className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 ${liked ? 'text-red-400' : 'text-white/40 hover:text-red-400'}`}
          >
            {liked ? '❤️' : '🤍'} {localLikes}
          </button>
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm text-white/40 hover:text-blue-400 font-medium transition"
          >
          ...
          <button
            onClick={handleShare}
            className="flex items-center gap-2 text-sm text-white/40 hover:text-green-400 font-medium transition"
          >
            🔗 {shares}
          </button>
        </div>
